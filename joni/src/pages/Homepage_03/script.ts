...
if (splitBlockMount) {
  ...
  const splitHeader = splitBlockMount.querySelector<HTMLElement>('.split-block__content h2');
  if (splitHeader) {
    splitHeader.innerHTML =
      'One platform.<br>Every capability you need';
  }
  const splitItems = Array.from(
    splitBlockMount.querySelectorAll<HTMLElement>('.split-block__item-title'),
  );
  const newTitles = ['Awareness', 'Consideration', 'Conversion', 'Loyalty', 'Advocacy'];
  splitItems.forEach((item, idx) => {
    if (newTitles[idx]) item.textContent = newTitles[idx];
  });

   const buttonSets: string[][] = [
     ['AI/GEO Visibility', 'Listings management', 'Local/location pages'],
     ['Reviews management', 'Social management', 'Messaging'],
     ['Chatbot', 'Marketing automation'],
     ['Surveys/VoC', 'Ticketing', 'Insights', 'Competitors'],
     ['Generate reviews', 'Referrals'],
   ];

   const panels = Array.from(splitBlockMount.querySelectorAll<HTMLElement>('.split-block__panel'));
   panels.forEach((panel, idx) => {
     panel.innerHTML = '';
     const labels = buttonSets[idx] || [];
     labels.forEach((label) => {
       const btn = document.createElement('a');
       btn.className = 'split-block__cta body-m';
       btn.href = '#';
       btn.textContent = label;
       const icon = document.createElement('span');
       icon.className = 'split-block__cta-icon';
       icon.dataset.icon = 'arrow.up.right';
