...
export const initPlatformDiagram = (scope: ParentNode = document) => {
  const lineImgs = scope.querySelectorAll<HTMLImageElement>('[data-graph-line]');
  const birdLogo = scope.querySelector<HTMLImageElement>('[data-birdai-logo]');
  ...
  const diagramRoot =
    scope instanceof HTMLElement
      ? scope
      : scope.querySelector<HTMLElement>('[data-platform-diagram]');
  if (!diagramRoot) return;

  const panels = Array.from(diagramRoot.querySelectorAll<HTMLElement>('[data-platform-panel]'));
  if (!panels.length) return;

  const setActivePanel = (panel: HTMLElement | null) => {
    if (panel) {
      diagramRoot.dataset.activePanel = panel.dataset.platformPanel || 'active';
    } else {
      delete diagramRoot.dataset.activePanel;
    }

    panels.forEach((item) => {
      if (panel && item === panel) item.dataset.active = 'true';
